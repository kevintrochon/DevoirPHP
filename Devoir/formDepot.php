...
<body>
    ...
    <form action="formDepot.php" enctype="multipart/form-data" method="post" style="padding-left:350px;">
        ...
        <div  class="h4">
          <input type="text" class="form-control" id="iduser" name="name" autofocus style="width: 250px;height:35px;" />
        </div>
        <div>
          <label for="iduser" class="h4" > Nom du document : </label>
        </div>
        <div  class="h4">
          <input type="text" class="form-control" id="iduser" name="nameDocu" style="width: 250px;height:35px;" placeholder="prescription, identitée,..." />
        </div>
        <div >
          <label for="typDocu" class="h4" > Type de document : </label>
        </div>
        <div  class="h4">
          <input type="text" class="form-control" id="typDocu" name="typDocu" style="width: 250px;height:35px;" placeholder="pdf, png, jpeg, jpg, gif,..." />
        </div>
          <div>
          <label for="Contenu" class="h4" > Contenu du document : </label>
        </div>
        <div  class="h4">
          <input type="text" class="form-control" id="Contenu" name="Contenu" style="width: 250px;height:35px;" placeholder="Radio, Certificat,..." />
        </div>
        <div>
            <label for="motifs" class="h4">Motif d'admission : </label>
            <!-- Sélectionneur de motif d'admission-->
            <select class="form-select form-select-sm" style="width: 250px;height:35px;" name="motif">
            <?php
    ...
    <br>
        <input type = "file" name = "upload" class="btn btn-outline-info"/><br/>
    </p>
    <p>
...
<?php
    ...
    if(isset($_POST['submit']) 
        ...
        if ($resultat) {
            if(enregistreDocument($_POST['name'],$_POST['nameDocu'],$pathDocument,$_POST['Contenu'],$_POST['typDocu'],getIdPatientByName($_POST['name']),$_POST['motif'])){
